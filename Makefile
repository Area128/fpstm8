...
size:
	@$(OBJCOPY) -I ihex --output-target=binary --strip-unneeded $(TARGET) $(TARGET).bin
	@echo "-----\nImage size:"
	@stat -L -c %s $(TARGET).bin

## @TODO: separate option-bytes for stm8s and stm8l!
# enable write-protection on first 20 pages
opt-set:
	@echo '0x00 0x13 0xf6 0x00 0xff 0x00 0xff 0x00 0xff 0x00 0xff' | xxd -r > opt.bin
